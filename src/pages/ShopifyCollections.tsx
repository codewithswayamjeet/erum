        ...
        <div className="container mx-auto px-6 lg:px-12">
          ...
          {isLoading ? <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 text-left">
                <CollectionCard title="Rings" description="Engagement, solitaire and everyday classics" image={collectionRings} href="/collections/rings" />
                <CollectionCard title="Necklaces" description="Pendants, chains and statement pieces" image={collectionNecklaces} href="/collections/necklaces" />
                <CollectionCard title="Earrings" description="Studs, hoops and chandelier drops" image={collectionEarrings} href="/collections/earrings" />
                <CollectionCard title="Bracelets" description="Tennis bracelets, bangles and more" image={collectionBracelets} href="/collections/bracelets" />
              </div>
            ...
            </div> : collections.length === 0 ? <div className="text-center py-16">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {collections.map(({
              node
            }) => <motion.div key={node.id} initial={{
              opacity: 0,
              y: 20
            }} whileInView={{
              opacity: 1,
              ...
              duration: 0.4
                    ...
                    <Link to={`/collections/${node.handle}`} className="group block">
                      <div className="relative aspect-[4/5] overflow-hidden bg-secondary mb-4">
                        <img src={node.image?.url || '/placeholder.svg'} alt={node.image?.altText || node.title} loading="lazy" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
