...
const BrandPromises = () => {
  const promises = [{
    ...
    details: ['Kimberley Process certified', 'Full supply chain traceability', 'Ethically mined from select sources', 'Zero tolerance for conflict diamonds']
  }, {
    icon: Gem,
    ...
    details: ['Responsible Jewellery Council certified', 'Recycled precious metals option', 'Fair labor practices', 'Environmental sustainability']
  }, {
    icon: Shield,
    ...
    details: ['Free lifetime repairs', 'Annual cleaning & inspection', 'Stone tightening service', 'Rhodium re-plating for white gold']
  }, {
    icon: Heart,
    ...
    details: ['Dedicated personal advisor', 'Worldwide shipping & insurance', 'Virtual consultations available', 'After-purchase care guidance']
  }, {
    icon: Award,
    ...
    details: ['Independent laboratory certification', 'Detailed grading reports', 'Laser inscription verification', 'Authenticity guarantee']
  }];
  const affiliations = [{
    name: 'GIA',
    ...
    name: 'GJEPC',
    description: 'Gem & Jewellery Export Promotion Council'
  }, {
    name: 'IBJA',
    description: 'India Bullion & Jewellers Association'
  }];
  return <Layout>
      {/* Hero Section */}
      ...
      <section className="py-24 bg-background">
        <div className="container mx-auto px-6 lg:px-12">
          <div className="space-y-20">
            {promises.map((promise, index) => <motion.div key={promise.title} initial={{
            opacity: 0,
            y: 40
          }} whileInView={{
            opacity: 1,
            ...
            delay: 0.1
                ...
                <div className={`bg-secondary p-8 lg:p-10 ${index % 2 === 1 ? 'lg:order-1' : ''}`}>
                  ...
                  <ul className="space-y-4">
                    {promise.details.map((detail, i) => <motion.li key={i} initial={{
                  opacity: 0,
                  x: -20
                }} whileInView={{
                  opacity: 1,
